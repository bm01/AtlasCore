...
namespace DOL.GS.PacketHandler.Client.v168
{
	...
	public class DetailDisplayHandler : IPacketHandler
	{
		...
		/// </summary>
		protected static readonly Logging.Logger log = Logging.LoggingManager.Create(MethodBase.GetCurrentMethod().DeclaringType);
				...
				case 7: //trade windows
					{
						...
						if (playerTradeWindow.PartnerTradeItems != null && objectId < playerTradeWindow.PartnerItemsCount)
							invItem = (DbInventoryItem)playerTradeWindow.PartnerTradeItems[objectId];
        ...
        public void WriteTechnicalInfo(IList<string> output, GameClient client, DbInventoryItem item)
		{
            WriteTechnicalInfo(output, client, item, item.Durability, item.Condition);
		}

		public void WriteTechnicalInfo(IList<string> output, GameClient client, DbInventoryItem item, int dur, int con)
		{
			output.Add(" ");
		...
		protected string GetShortItemInfo(DbInventoryItem item, GameClient client)
		{
		...
		/// - X.X Factor
		/// </summary>
		public void WriteClassicArmorInfos(IList<string> output, DbInventoryItem item, GameClient client)
		{
			output.Add(" ");
			output.Add(" ");
		...
		public void WriteMagicalBonuses(List<string> output, DbInventoryItem item, GameClient client, bool shortInfo)
		{
			...
			if (!shortInfo)
			{
				...
				if (item.ProcSpellID != 0)
				{
					...
					Spell procSpell = SkillBase.GetSpellByID(item.ProcSpellID);
				...
				if (item.ProcSpellID1 != 0)
				{
					...
					Spell procSpell = SkillBase.GetSpellByID(item.ProcSpellID1);
		...
		protected void WriteHorseInfo(IList<string> list, DbInventoryItem item, GameClient client, string horseName)
		{
			WriteHorseInfo(list, item.Template, client, horseName);
		}
		
		protected void WriteHorseInfo(IList<string> list, DbItemTemplate item, GameClient client, string horseName)
		{
			list.Add(" ");
			list.Add(" ");
			if (item.Level <= 35)
		...
		/// <param name="client"></param>
		private static void WritePotionInfo(IList<string> list, DbItemTemplate item, GameClient client)
		{
			WritePotionInfo(list, GameInventoryItem.Create(item), client);
		}

		private static void WritePotionInfo(IList<string> list, DbInventoryItem item, GameClient client)
		{
			if (item.SpellID != 0)
			{
