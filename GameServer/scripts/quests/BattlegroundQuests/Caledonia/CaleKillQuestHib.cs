...
namespace DOL.GS.DailyQuest.Hibernia
{
	public class CaleKillQuestHib : Quests.DailyQuest
	{
		...
		// Constructors
		public CaleKillQuestHib() : base()
		{
		}

		public CaleKillQuestHib(GamePlayer questingPlayer) : base(questingPlayer)
		{
		}

		public CaleKillQuestHib(GamePlayer questingPlayer, int step) : base(questingPlayer, step)
		{
		}

		public CaleKillQuestHib(GamePlayer questingPlayer, DbQuest dbQuest) : base(questingPlayer, dbQuest)
		{
		}
		...
		private static void TalkToPazz(DOLEvent e, object sender, EventArgs args)
		{
			...
			if(PazzHib.CanGiveQuest(typeof (CaleKillQuestHib), player)  <= 0)
				return;

			//We also check if the player is already doing the quest
			CaleKillQuestHib quest = player.IsDoingQuest(typeof (CaleKillQuestHib)) as CaleKillQuestHib;
			...
			else if (e == GameLivingEvent.WhisperReceive)
			{
				...
				if (quest == null)
				{
					...
					{
						case "help me out":
							player.Out.SendQuestSubscribeCommand(PazzHib, QuestMgr.GetIDForQuestType(typeof(CaleKillQuestHib)), "Will you undertake " + questTitle + "?");
							break;
					}
				}
				else
		...
		public override bool CheckQuestQualification(GamePlayer player)
		{
			...
			if (player.IsDoingQuest(typeof (CaleKillQuestHib)) != null)
				return true;
		...
		private static void SubscribeQuest(DOLEvent e, object sender, EventArgs args)
		{
			...
			if (qargs.QuestID != QuestMgr.GetIDForQuestType(typeof(CaleKillQuestHib)))
				return;
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
				...
				if (!PazzHib.GiveQuest(typeof (CaleKillQuestHib), player, 1))
					return;
