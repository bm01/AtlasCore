...
{
	public class ThidKillQuestAlb : Quests.DailyQuest
	{
		/// <summary>
		...
		// Constructors
		public ThidKillQuestAlb() : base()
		{
		}

		public ThidKillQuestAlb(GamePlayer questingPlayer) : base(questingPlayer)
		{
		}

		public ThidKillQuestAlb(GamePlayer questingPlayer, int step) : base(questingPlayer, step)
		{
		}

		public ThidKillQuestAlb(GamePlayer questingPlayer, DbQuest dbQuest) : base(questingPlayer, dbQuest)
		{
		}
		...
		public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
		{
			...
			PazzAlb.AddQuestToGive(typeof (ThidKillQuestAlb));
		...
		public static void ScriptUnloaded(DOLEvent e, object sender, EventArgs args)
		{
			...
			PazzAlb.RemoveQuestToGive(typeof (ThidKillQuestAlb));
		}
		...
		protected static void TalkToRey(DOLEvent e, object sender, EventArgs args)
		{
			...
			if(PazzAlb.CanGiveQuest(typeof (ThidKillQuestAlb), player)  <= 0)
				return;

			//We also check if the player is already doing the quest
			ThidKillQuestAlb quest = player.IsDoingQuest(typeof (ThidKillQuestAlb)) as ThidKillQuestAlb;
			...
			else if (e == GameLivingEvent.WhisperReceive)
			{
				...
				if (quest == null)
				{
					...
					{
						case "help me out":
							player.Out.SendQuestSubscribeCommand(PazzAlb, QuestMgr.GetIDForQuestType(typeof(ThidKillQuestAlb)), "Will you undertake " + questTitle + "?");
							break;
					}
				}
				else
		...
		public override bool CheckQuestQualification(GamePlayer player)
		{
			...
			if (player.IsDoingQuest(typeof (ThidKillQuestAlb)) != null)
				return true;
		...
		private static void SubscribeQuest(DOLEvent e, object sender, EventArgs args)
		{
			...
			if (qargs.QuestID != QuestMgr.GetIDForQuestType(typeof(ThidKillQuestAlb)))
				return;
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
				...
				if (!PazzAlb.GiveQuest(typeof (ThidKillQuestAlb), player, 1))
					return;
		...
		public override string Description
		{
			get
			{
				...
				{
					case 1:
						return "You will find suitable players in Thidranki. \nPlayers Killed: ("+ PlayersKilled +" | "+ MAX_KILLED +")";
					case 2:
